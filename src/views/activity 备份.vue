<template>
    ...
    <div class="header" :class="[{'header-kj':isNew==0},
      ...
      <i style="position: absolute; right: 0.2rem; font-size: 0.34rem;" v-show="$store.state.isPure!=1" @click="$router.push('/newTask')">新手任务</i>
    </div>
    <div class="yh-menu">
         <myScrollX >
    ...
    >
        <ul v-if="dataList.length" class="">
            <li v-for="(item,index) in dataList" :key="index" @click="yhdetail(item)">
                <span> <i :style="item.ishot == 1 ?'color:#60353D':'color:#fff'"> {{item.type_text}}</i><img v-lazy="item.type_icon" alt=""></span>
