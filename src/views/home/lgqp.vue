        ...
        methods:{
            findClose(d){
             ...
             async getHotGame(flag) {
                  let allList = this.$store.state.gameCenterLgcp;
                  if(allList.length){
                ...
                let res = await this.$http.get("/api2/user/all-games?style=1&type=fh_chess");
                this.$vux.loading.hide();
                if (res.data && res.data.code == 1) {
                    this.$store.commit('SETGAMECENTERLGCP',res.data.data.list || [])
