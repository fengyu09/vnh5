    ...
    >
      ...
      <div class="item-box clearfix" v-for="(item,index) in lotteryList" :key="index">
        <div class="title clearfix">
          ...
          <span class="fr" @click="seeDatail(item.game_type,item.game_name,item.tag)">kiểm tra các chi tiết <img class="fr-icon" src="../../assets/images/extend/right@2x.png" alt=""></span>
        </div>
        <div class="item">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      seeDatail(id,name,tag){
        this.$router.push('/cqDetail/'+id+'?name='+name+'&tag='+tag);
        // localStorage.setItem('st',this.st);
        // localStorage.setItem('et',this.et);
      },
      async getLotteryReport(s,e){
        ...
        localStorage.setItem('et',this.et);
        this.isLoading = true;
        let res = await this.$http.get('/api/vendors.cq9/detailcount',{
          params:{
            st:s || this.st,
            et:e || this.et,
            // game_type:'sport'
