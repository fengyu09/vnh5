<template>
  <div class="QPGame">
    ...
    <myScroll
            ...
            :data="playList"
    >
      <div class="main-box" :class="$st.state.skin==5?'main-box-golden':''">
      <div class="item-box clearfix" v-for="(item,index) in lotteryList" :key="index">
        <div class="title clearfix">
            <span class="fl">
              {{item.game_name}}
            </span>
          <span class="fr" @click="seeDatail(item.game_id)">{{$t('gameCount[0]')}}  <img class="fr-icon" src="../../assets/images/extend/right@2x.png" alt=""></span>
        </div>
        <div class="item">
          <span>{{$t('gameCount[1]')}} </span>
          <strong>{{item.count}}</strong>
        </div>
        <div class="item">
          <span>{{$t('gameCount[2]')}} </span>
          <strong>{{item.amount}}</strong>
        </div>
        <div class="item">
          <span>{{$t('gameCount[3]')}} </span>
          <strong class="red" :style="$st.state.skin==5?'color: #AB9B62;':''">{{item.prize}}</strong>
        </div>
        <div class="item">
          <span>{{$t('gameCount[4]')}}  ({{$t('moneyDw')}})</span>
          <strong :class="item.profit>0?'red':''" :style="$st.state.skin==5?'color: #AB9B62;':''"><i v-if="item.profit>0">+</i>{{item.profit}}</strong>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      seeDatail(id,type){
        // this.$router.push('/aeLiveDetail/'+id);
        this.$router.push('/gameCountDetail?gameId='+id+'&code='+this.code+'&gameType='+this.gameType);
      ...
      async getLotteryReport(s,e){
        ...
        localStorage.setItem('et',this.et);
        this.isLoading = true;
        let res = await this.$http.get('/api/vendors.'+this.code+'/detailcount',{
          params:{
            st:s || this.st,
            et:e || this.et,
