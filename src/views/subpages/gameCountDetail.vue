<template>
    <div>
        ...
        >
            <div class="zd-content" :class="$st.state.skin==5?'zd-content-golden':''">
                <table class="">
                    <tr class="tab-th">
                        <td class="date">{{$t('gameCountDetail[0]')}}</td>
                        <td class="issue">{{$t('gameCountDetail[1]')}}</td>
                        <td class="number">{{$t('gameCountDetail[2]')}}</td>
                        <td class="money">{{$t('gameCountDetail[3]')}} ({{$t('moneyDw')}})</td>
                    </tr>
            ...
            <p class="no-data-tip" v-if="deadline&&!isLoading&&recordList.length&&recordList.length>limit">{{$t('deadline')}}~</p>
        </myScroll>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created(){
            this.currLoId = this.$route.query.gameid;
        ...
        methods: {
            ...
            async loadMore(id,st,et) {
                ...
                this.isLup = true;
                this.page++;
                let res = await this.$http.get("/api/vendors."+this.code+"/gamerecord", {
                    params: {
                        limit: this.limit,
                        page: this.page,
