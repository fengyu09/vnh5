    ...
    <div id="xz" :class="[{'xzTab-pink':skin==1},{'xzTab-blue':skin==2},{'xzTab-green':skin==3},{'xzTab-maingreen':skin==4},{'xzTab-golden':skin==5}]">
       ...
       <div class="content">
           ...
           <div class="wx-tip clearfix">
             <div class="fl">
               ...
               <div class="czlb fl" style="margin-left:5px;" >
                  <div class="cz-name" :class="[{'skin-border-b-pink':skin==1},{'skin-border-b-blue':skin==2},{'skin-border-b-green':skin==3},{'skin-border-b-maingreen':skin==4}]" @click="goGamePlay(currLottery)"> {{currLottery.vname}} <i class="icon fr"></i></div>
               </div>
             </div>
              <!-- pk3的 下一期的期数-->
             ...
             <div class="fr " v-else  :class="showDate== true ?'up-icon':'down-icon'" @click="showDate = !showDate"></div>
            ...
            <div class="cm-list clearfix" v-if="!Array.isArray(currData)&&(cid<45)" :class="'lott'+(index+1)" v-for="item,index in currData.field_def" :key="index">
               ...
               <div class="list-r fr clearfix">
                  ...
                  <div class="list-content clearfix">
                    <span class="fl" :data-ball="num.code" :data-ballxs="num.name" :data-ws="item.prompt"  @click="checkBall($event)" :data-pcname='num.code' :data-index="numIndex" v-for="(num,numIndex) in item.nums" :key="numIndex">{{num.name}}</span>
                    <!-- <i>1</i> -->
                  </div>
               </div>
            </div>
         ...
         <div class="ks-list" v-if="tabIndex == 'direct'">
            ...
            <div class="kx-list" v-if="(currBigwf=='combo'||currBigwf=='lose')">
              <ul class="clearfix">
                <li class="fl" v-for="ball,index in singleData" :key="index" v-show="currMaxNum.max>=index&&currMaxNum.min<=index" @click="selectOneBall($event,ball)"  :class="{'active':singleSelectNumber.findIndex(v=>{return v.item_code ==ball.code})>-1,'no-active':singleSelectNumber.length==maxComboLen}">{{ball.name}}</li>
            ...
            <div class="kx-list" v-else>
              <ul class="clearfix">
                <li class="fl" v-for="ball,index in singleData" :key="index" v-show="currMaxNum.max>=index&&currMaxNum.min<=index" @click="selectOneBall($event,ball)" :class="singleSelectNumber.findIndex(v=>{return v.item_code ==ball.code})>-1?'active':''">{{ball.name}}  </li>
                <!-- <li class="fl active">00<i>4</i></li> -->
              </ul>
            </div>
          </div>
       ...
       <div class="title-list-box show-box" v-if="showTitle" :class="skin==5?'title-list-box-golden':''">
           <div class="title-list" :class='[{"title-pink":skin==1},{"title-blue":skin==2},{"title-green":skin==3},{"title-maingreen":skin==4},{"title-golden":skin==5}]'>
             <myScroll :top="skin==5?0:0.4" :bottom="4" :bgColor="skin==5?'1F252C':'fff'" ref="menuScroll">
       ...
       <div class="show-box" v-if="isXzComfirm">
         <div class="qr-box">
           ...
           <div style="padding:.3rem .56rem 0 .56rem">
             ...
             <span> {{$t('xz.text[28]')}}:  <i>{{bnum*chipsJE*mult}}</i> vndk</span>
             <span v-if="(!Array.isArray(currData)&&currData.prize.length)&&!(cid>=45&&cid<=50)">{{$t('xz.text[29]')}} <i style="color:#FF5640">{{mult*currData.prize[0].prize}}</i> vndk</span>
             <!-- <span v-else> Tiền thắng /1 lần: <i style="color:#FF5640">{{mult*currData[0].prize[0].prize}}</i> vndk</span> -->
           </div>
        ...
        <div class="quick-content">
          <div class="user-info" :class="[{'user-info-pink':skin==1},{'user-info-blue':skin==2},{'user-info-green':skin==3},{'user-info-maingreen':skin==4},{'user-info-golden':skin==5}]">
            ...
            <div class="clearfix">
              ...
              <span class="money">{{amount}}VNDK</span>
            </div>
            <div class="btn-group">
      ...
      this.contents = {
        ...
        if(this.chatShow){
         ...
         //正常的直选
          ...
          //获取最新开奖数据（当前期）
          async getCurrOpenData(flag,flag2){
             let res = {}
            if(flag){
