<template>
  <div class="vip-details">
    ...
    <myScroll :top="0" :bottom="0" :data="playList" ref="scrollWrapper">
      ...
      <div class="vip-content">
        <table class="vip-tj">
        <tr>
          <td style="width: 50%">晋级条件</td>
          <td>Tiền gửi tích lũy：<font style="color: red">{{CurrvipDetail.cz_total |filterOnlineNum}}</font> <br>
            -
            <br>
            累计流水：<font style="color: red">{{CurrvipDetail.flow_total|filterOnlineNum}}</font>
          </td>
        </tr>
        <tr>
          <td>保级条件</td>
          <td>流水（一个月）：<font style="color: red">{{CurrvipDetail.rg_flow|filterOnlineNum}}</font></td>
        </tr>
      </table>
      <table class="vip-jl">
        <tr>
          <td style="width: 50%;">升级礼金</td>
...
export default {
  ...
  methods:{
    ...
    async getVipDetail(){
      let res = await this.$http.get(this.versionLive2+'user/get_vip_detail/');
      if(res && res.data.code==1){
          this.CurrvipDetail = res.data.data.vip_list[this.currVip==0?0:this.currVip-1];
          this.vipDetailList = res.data.data.vip_list;
          this.tip = res.data.data.tips;
...
.vip-detail-img{
   background: url("../../assets/images/vipRanking/bj.jpg") #000 top  no-repeat;
  background-size: 100% 100%;
  padding-top: 5.5rem;
  padding-bottom: 1.5rem;
  min-height: 46rem;
