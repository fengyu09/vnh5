      ...
      <table width="100%">
        <thead>
          <tr v-if="rankNum==11">
            ...
            <th v-for="index in 19" :ref="'refth'+index" v-if="index>2" :key="index+'x'">{{index}}</th>
          </tr>
          <tr v-else>
            <th width="20%">期号</th>
            <th width="8%" ref="refnum">号码</th>
            <th v-for="index in 10" :ref="'refth'+index" :key="index+'x'">{{index}}</th>
          </tr>
        </thead>
        <tbody v-if="configKonva.length">
          <tr v-for="(item,index) in openInfo" :key="index">
            ...
            <td class="color-red" style="position: relative;">
              <span v-if="rankNum==11">{{item.trending.findIndex(i=>i==0)+3}}</span>
              <span v-else>{{item.trending.findIndex(i=>i==0)+1}}</span>
            </td>
            <td style="position: relative;" v-for="(item2,index2) in item.trending" :key="index2">
              <span :class="item2==0?'hotSpan':0">
                <span v-if="rankNum==11">{{item2==0?index2+3:item2}}</span>
                <span v-else>{{item2==0?index2+1:item2}}</span>
              </span>
              <v-stage
                v-if="item2==0&&index<openInfo.length-1"
...
export default {
  ...
  mounted() {
    ...
    if (this.rankNum == 11) {
      this.tdWidth = Math.max(
        this.$refs.refth8[0].offsetWidth,
        this.$refs.refth16[0].offsetWidth
      );
    }
