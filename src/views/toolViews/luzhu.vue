<template>
  <div>
    <div
      ...
      v-if="item1.length"
      v-show="rankNumShow.indexOf(index1+1)!=-1"
    >
      <div class="clearfix pearl-day">
        <span class="fl " :class="[{'pearl-day-bd-dw':isNew == 2},{'pearl-day-bd':isNew == 0 || isNew == 1 ||isNew == 2},{'pearl-day-bd-gq':isNew == 4},{'pearl-day-bd-sdj':isNew == 5},{'pearl-day-bd-ox':isNew == 6},{'pearl-day-bd-uefa':isNew == 7}]">今日：</span>
        <span v-if="lzType=='daxiao'">
          <span class="fl">大</span>
          <span class="fl">( {{statistics[index1].da}} )</span>
          <span class="fl">小</span>
          <span class="fl">( {{statistics[index1].xiao}} )</span>
        </span>
        <span v-if="lzType=='danshuang'">
        ...
        <span v-if="lzType=='longhu'">
          ...
          <span class="fl" v-if="statistics[index1].he!=undefined">和</span >
          <span class="fl" v-if="statistics[index1].he!=undefined">( {{statistics[index1].he?statistics[index1].he:0}} )</span>
        </span>
        ...
        <span
          class="fr"
          v-if="rankNum!=0&&lzType!='gy'&&lzType!='zonghe'&&lzType!='num'&&lzType!='qianhou'"
        >
          <span v-if="[1,8,10].find(res=>res==zsid)">{{'第'+(index1+1)+'球'}}</span>
          <span v-else>{{index1==0?'冠军':'第'+(index1+1)+'名'}}</span>
        </span>
        <span class="fr" v-if="lzType=='zonghe'">总和</span>
        <span class="fr" v-if="lzType=='num'">{{'号码'+index1}}</span>
        <span class="fr" v-if="lzType=='qianhou'">{{'号码'+(index1+1)}}</span>
      </div>
      <myScrollX ref="scrollWrapper" style="overflow: hidden;">
        <div
          class="pearl-list"
          ref="pearlList"
          :style="{width:item1[0].length*0.6+'rem','float':item1[0].length<17?'right':''}"
        >
            ...
            <tbody>
              <tr v-for="(item,index) in item1" :key="index">
                <td v-for="item2 in item">
                  <span
                    v-if="lzType!='num'"
                  >{{item2!=''?item2.substring(item2.length-1,item2.length):''}}</span>
                  <span v-else>
                    <img
                      src="../../assets/images/hmlz-icon2.png"
                      v-if="item2.substring(item2.length-1,item2.length)=='0'"
                      style="width:0.30rem"
                    />
                    <img
                      src="../../assets/images/hmlz-icon1.png"
                      v-if="item2.substring(item2.length-1,item2.length)=='1'"
                      style="width:0.32rem"
                    />
          ...
          if (res.data.code == 1) {
            ...
            this.$nextTick(() => {
              this.luzhuList.forEach((item, index) => {
                let scrollWidth =
                  this.$refs.pearlList[index].offsetWidth - window.innerWidth;
                if (scrollWidth > 0) {
                  setTimeout(() => {
...
// .pearl-list table tr td:nth-child(even){
//   color: #ff513e;
// }
</style>
