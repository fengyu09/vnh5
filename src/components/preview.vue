    ...
    <div class="one-item" v-for="(v,k) in LivePreview" :key="k">
      ...
      <ul class="preview-list-box clearfix">
        <li v-for="(item,index) in v" :key="index">
          ...
          <div class="hot-icon" v-if="index<3&&isHot&&item.livestatus==1&&(k.split(' ')[0]=='今天')" :class="'hot-icon'+(index+1)"></div>
        </li>
      </ul>
    </div>
  ...
  methods: {
    ...
    async getPreview(n) {
      ...
      let res = await this.$http.get(
        this.versionLive2 + "/user/anchor_advance/",
        {
          params: {
    ...
    .preview-list-box {
      li {
        ...
        .focus-btn {
          ...
          &.focus {
            background: url("../assets/images/chat/yiguanzhu@2x-min (1).png")
